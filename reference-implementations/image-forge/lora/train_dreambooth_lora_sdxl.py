...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if args.dataset_name is not None:
            ...
            # image-forge patch: a local directory of images + metadata.jsonl must be
            # loaded with the explicit "imagefolder" builder, else datasets reads only
            # the metadata (no `image` column). Hub names use the normal path.
            if os.path.isdir(args.dataset_name):
                dataset = load_dataset(
                    "imagefolder", data_dir=args.dataset_name, cache_dir=args.cache_dir,
                )
            else:
                dataset = load_dataset(
                    args.dataset_name,
