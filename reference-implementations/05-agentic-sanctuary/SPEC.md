# Build #5 — The Agentic Sanctuary — SPEC

Normative specification for the reference implementation of **book ch. 35**. Keywords
**MUST**, **SHOULD**, **MAY** are used in the RFC-2119 sense.

This build **forks Build #4 in place**: the body, the room, the voice loop, the hands,
and the one outbound bus are this build's own code, carried forward file-for-file (see
`B4-FORK.md` for provenance and the re-diff command), and **Build #4's SPEC remains the
normative text for that machinery** — cited here as **B4 §n**
(`../04-3d-world-companion/SPEC.md`), exactly as Build #4 cited **B2 §n** and **B1 §n**
for its vendored layers. To keep every inherited file's `SPEC §n` comment meaningful,
**§1–§14 of this document mirror Build #4's section numbering** — each section either
adopts B4's text by citation or states its amendment. The new work — the mind — is
**§15–§25**, with omissions, tests, and the growth path at §26–§28.

---

# Part I — the chassis (§1–§14): Build #4, amended

## §1 — Goal and properties

The capstone of the build ladder: Build #4's browser-based 3D companion — the VRM body
in the canonical sanctuary, the chat beside her, the real-time voice loop, four MCP
tools, one event bus — now driven by an **always-on autonomy engine**. She runs
...
```
 ...
 ┌────────────────────────────────────────────────────────────────────────────────────┐
 │  Build #4's chassis, forked in place (B4 §2–§10):                                  │
...
B4 §2 applies: the Build #1 brain is vendored in `./app`, the Build #2 voice stack in
`./desktop`, the image-forge slice in `./forge`, the SOUL source in `./soul-src`;
`ToolBrain` subclasses the vendored adapter; nothing points at a sibling build.
Amendments:
...
**by citation, unchanged**. The one contractual amendment: B4 §4 promised the
controller's method surface as "the strings Build #5's tick loop will hold" — §15.5
holds it to that.
