...
The vendored `app.config.Config` (the Build #1 brain, bundled here so Build #2
runs standalone — see app/VENDORED.md) already holds every brain knob (model,
Vault, prompt/memory budgets). This subclass adds the *voice* knobs and flips the
brain defaults that matter for Build #2:
...
from app.config import Config as BrainConfig
...
class Config(BrainConfig):
    ...
    soul_src: Path = Path("./soul-src")        # vendored SOUL, for seeding
