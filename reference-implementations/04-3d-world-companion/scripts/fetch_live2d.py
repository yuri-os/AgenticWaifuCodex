...
Build #2's `scripts/fetch_avatar.py`, vendored (SPEC §6.6): identical except
for the destination — the Live2D client lives under web/live2d/ here, next to
Build #4's own VRM frontend — and the closing hint. If you change the fetch
logic, change it in Build #2 and re-copy (see web/live2d/VENDORED.md).
...
def main() -> None:
    ...
    if not args.skip_cdn_samples:
        install_cdn_samples()
    print("\nDone. `python -m world` → open /live2d/ in a browser, or float her:\n"
          "`python -m world --window --body live2d`.\n"
