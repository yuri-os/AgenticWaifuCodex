...
from desktop.voice.backends.fakes import FakeBrain, _wordish   # noqa: E402
...
def test_voice_transcript_keeps_the_authored_cold_open(cfg, controller):
    ...
    with TestClient(app) as client:
        with client.websocket_connect("/ws/voice") as ws:
            ...
            ws.receive_json()
            drain(ws)
        assert app.state.rt.transcript[0]["text"] == brain.cold_open()
...
def test_ambient_injection_reaches_the_client_and_is_not_persisted(rig):
    ...
    with client.websocket_connect("/ws/voice") as ws:
        ...
        # the idle machine speaks (from the server side, through the fork's seam)
