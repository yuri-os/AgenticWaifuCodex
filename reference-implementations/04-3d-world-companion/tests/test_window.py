...
inside run()), reuse the vendored B2 launcher helpers rather than copy them,
...
the flag, and the vendored Live2D client must be served with its two API needs
...
@pytest.fixture
def client(cfg):
    cfg = cfg.model_copy(update={"tools_backend": "off", "idle_enabled": False})
    app = create_app(cfg, brain=FakeBrain())
    # loopback client host so the vendored settings router's local-only gate opens
...
def test_reuses_the_vendored_launcher_helpers():
    # §2.2's discipline: vendored code is called, not copied — the readiness
...
# ---- the second body: the vendored Live2D client (§6.6) -----------------------

def test_live2d_client_is_vendored_and_served(client):
    for name in ("index.html", "avatar.js", "voice.js", "settings.js",
                 "sanctuary.css", "VENDORED.md"):
...
def test_vendored_settings_router_answers_here_too(client):
