"""The situation block (SPEC §2.5) — she knows when and where she is.

Ch. 19's world model, at Build #4 scale: not a `WorldModelStore`, just the
present tense rendered fresh into every prompt. Without it the build fails two
questions it has no right to fail. "What time is it?" — the honesty constraint
(B1 §7.4) rightly forbids inventing facts, and no block carried the clock, so
she says she doesn't know while the host runs her timers to the second. And
"blink for me" — a safety-aligned base model's reflex is "I'm an AI, I have no
body," which is cold and, here, simply false: her body is rendered on screen
this instant, blinking on the idle machine's clock. She may know she is an AI;
she is never bodiless.

Everything below reads surfaces the host already owns — the injected clock,
the controller's sticky scene state, the timer board — so the block costs no
new state. Build #5 replaces this rendering with a real
`WorldModelStore.situation()` (→ ch. 19); the block's *place* in the prompt is
