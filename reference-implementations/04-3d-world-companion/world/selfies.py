...
The generator behind it is the vendored forge (./forge, → ch. 26): the locked
...
def build_forge(cfg) -> tuple[object, str]:
    ...
    from forge import Character, ImageForge, SelfieBook, make_backend
    ...
    async def _job(self, c: dict) -> None:
        ...
        try:
            result = await asyncio.to_thread(
                self.forge.selfie, scene=scene, mood=mood, save=False)
