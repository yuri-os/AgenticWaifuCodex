"""GET /api/config — the rig registry for the vendored Live2D client (SPEC §6.6).

Build #2's whole web client is vendored under web/live2d/ and served as-is; its
avatar.js asks `/api/config` which Live2D rig to wear (AVATAR_MODEL in .env,
resolved through the vendored registry `desktop/avatar_models.py` — called, not
...
router = APIRouter()
LIVE2D_DIR = Path(__file__).resolve().parent.parent.parent / "web" / "live2d"
