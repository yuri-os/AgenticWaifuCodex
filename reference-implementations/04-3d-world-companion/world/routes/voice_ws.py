...
FORK(B2 §10): this file is a copy of the vendored `desktop/routes/voice_ws.py`
with a small set of additions, each marked `FORK(B2 §10)` below:
  ...
  1. engagement notifications — `rt.turn_started()` / `rt.turn_ended()` around
     each turn's pump, so the idle machine knows when she's talking (SPEC §8.4);
  2. the ambient injector — this connection registers a coroutine that lets the
     idle machine speak a cue *through this connection's TurnController*, so
     ambient speech gets the same OutEvent stream, the same barge-in cancel,
     and the same latency masking as a real turn (SPEC §8.3–§8.4);
...
from desktop.voice.turn import OutEvent, TurnController
...
async def voice(ws: WebSocket):
    ...
    async def run(agen, proactive: bool = False) -> None:
        ...
        rt.turn_started()                          # FORK(B2 §10): idle machine (§8.4)
    ...
    # connection: same TurnController, so a barge-in cancels ambient speech the
    # same way it cancels a reply. Returns False when a turn is already in
    # flight — the idle machine treats that as "she's busy".
