"""Build #4 — the 3D world companion (SPEC §1, §2).

One process, one origin: the vendored Build #1 brain and Build #2 voice loop,
the `EventHub` carrying every host→frontend event (chat, drafts, her body's
puppet commands — SPEC §10, the YuriOS shape), the MCP tool loop, the timer
board, the selfie lab, and the idle machine — composed here, served with the
sanctuary frontend. Run:

    python -m world                        # reads HOST/PORT from .env (§11)

The voice stack still warms off-thread (B2's pattern — her body renders in
seconds, her voice follows), and the new async machinery (tool runner, timers,
idle) starts on FastAPI startup so it lives on the server's event loop.
"""
...
from desktop.main import build_stt, build_tts, build_vad
...
class Runtime:
    ...
    def __init__(self, cfg: Config, *, brain=None, chat_model=None,
        ...
        # are declared here and resolved on the warm-up thread; tools/idle on the
        # event loop (start_async); selfies is known now. /api/boot serves it.
        self.boot = BootBoard()
        ...
        # the vendored BrainAdapter with the §7 tool loop wrapped around it.
        ...
        self.idle: IdleMachine | None = None
        self.idle_status = "disabled"

        # declare the boot services in the order they should read down the panel.
        # voice comes up on the warm-up thread below; tools/idle are resolved in
        ...
        self.boot.declare(
            "idle", "presence · idle life",
            state="pending" if cfg.idle_enabled else "skipped")
        # per-connection ambient injectors (SPEC §8.4): session_id → coroutine fn.
        # The idle machine speaks *through a live voice connection* so barge-in
        # and the OutEvent stream work exactly as they do for a real turn.
    ...
    def post_message(self, role: str, text: str, *, image_url: str | None = None,
        ...
        finished selfie) — the YuriOS flag, same meaning."""
    ...
    # ---- engagement notifications from the voice route (SPEC §8.4) ----

    def turn_started(self) -> None:
        if self.idle:
            self.idle.turn_started()

    def turn_ended(self) -> None:
        if self.idle:
            self.idle.turn_ended()
    ...
    async def start_async(self) -> None:
        ...
        if self.cfg.idle_enabled:
            self.idle = IdleMachine(self.cfg, self.clock, self.controller,
                                    self.timers, self.speak_ambient)
            self.idle_status = "running"
            self.boot.done("idle", detail="running")
            self._tasks.append(asyncio.create_task(self.idle.run(), name="idle"))
...
def build_server(app: FastAPI, cfg: Config):
    ...
    -m world`, the desktop window, the demo). Sets the graceful-shutdown cap and
...
def create_app(cfg: Config | None = None, *, brain=None, chat_model=None,
    ...
    app = FastAPI(title="world-companion", docs_url=None, redoc_url=None,
                  lifespan=lifespan)
    app.state.rt = rt
    ...
    from desktop.routes import settings as b2_settings

    from .routes import events, health, live2d, voice_ws
    app.include_router(health.router)
    app.include_router(events.router)
    app.include_router(voice_ws.router)
    # The second body (SPEC §6.6): Build #2's Live2D client, vendored under
    ...
    # vendored settings router (called, not copied — it edits THIS build's
