"""`python -m world --window` — her on the desktop, the room set aside (SPEC §6.5).

Build #2 proved the frame (the vendored desktop/window.py, B2 §9): run the
...
from the vendored module*, not copied; what B2's run() learned the hard way is
...
inherited from the vendored config; 360×640 portrait suits a standing VRM body
...
def desktop_url(cfg: Config) -> str:
    ...
    vendored Build #2 Live2D client at `/live2d/` — both already carry their own
...
def run(cfg: Config | None = None) -> None:
    ...
    # documented in the vendored desktop/window.py).
    if _wait_for_server(cfg.host, cfg.port, timeout=0.5):
        raise SystemExit(
            f"port {cfg.port} is already in use — an earlier `python -m world` is "
            "probably still running. Close it (e.g. pkill -f 'python -m world') "
