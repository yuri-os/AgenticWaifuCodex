"""ToolBrain (SPEC §2.3, §7.4) — the vendored Build #2 brain adapter, given hands.
...
vendored `desktop.brain.BrainAdapter`, called, not copied.
...
is the price of keeping the vendored brain byte-identical — and the seam where
...
from desktop.brain import BrainAdapter
...
class ToolBrain(BrainAdapter):
    ...
    # -- prompt assembly: the vendored blocks + the situation (SPEC §2.5) ------
    def _assemble(self, session_id: str, text: str, *, window: list[dict],
                  lore) -> object:
        """B2's assembly, then the present tense appended — so every prompt
        (reply, greeting, ambient self-talk) knows when and where she is. The
        clock is the guard's injected one, never the wall clock (§8.2)."""
        soul, prompt = super()._assemble(session_id, text, window=window, lore=lore)
        prompt.messages[0]["content"] += (
            "\n\n## THE SITUATION RIGHT NOW\n\n"
            + render_situation(self.guard.clock, controller=self.controller,
    ...
    async def stream_reply(self, session_id: str, text: str) -> AsyncIterator[str]:
        # bookkeeping mirrors the vendored BrainAdapter.stream_reply line for line
        # (B2 §2.2 — the base body streams directly, so the override restates it)
        from desktop.brain import _Pending
