...
from app.memory import summarise
...
async def chat(req: ChatRequest, request: Request):
    ...
    async def stream():
        ...
        try:
            ...
            # A mid-stream/model bookkeeping failure writes no partial turn.
            log.exception("model stream failed")
            yield sse({"error": str(e)})
        finally:
            # The client may close an SSE stream without raising in this generator.
            # Only the pre-appended user message is provisional at that point.
            if not completed:
