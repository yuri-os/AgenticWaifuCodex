#!/usr/bin/env python3
"""Populate web/vendor/ with the Live2D runtime + the Hiyori model (SPEC §8.2).
...
redistributing them (exactly how AIRI keeps them in a .cache). Run once:
    ...
    python scripts/fetch_avatar.py                 # model from the local AIRI checkout
    python scripts/fetch_avatar.py --model-zip path/to/hiyori_free_zh.zip

What lands in web/vendor/:
...
avatar is skipped when vendor/ is empty (web/avatar.js says so out loud).
"""
...
def main() -> None:
    ...
    if not args.skip_cdn_samples:
        install_cdn_samples()
    print("\nDone. `python -m desktop` and open the sanctuary — she has a body now.\n"
