...
# Build #1 (the brain), vendored into ./app so Build #2 runs standalone — the
# exact code ch. 31 walks through (see app/VENDORED.md).
from app.core import assemble as asm
from app.corpus import CorpusLogger  # noqa: F401 (documents the reused surface)
...
class BrainAdapter:
    ...
    async def persist(self, session_id: str, user_text: str, reply: str) -> None:
        ...
        # From here the turn is durably represented in the transcript; errors in
        # the slower post-turn work must not roll its user half back.
