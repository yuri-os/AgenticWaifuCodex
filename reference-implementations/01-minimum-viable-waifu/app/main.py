...
from app.corpus import CorpusLogger, UtilityLogger
from app.memory.store import FileMemoryStore
from app.routes import chat, greeting, health, rate, session
...
def _ensure_index_matches_embedder(store, cfg: Config) -> None:
    ...
    from app.memory.reindex import reindex, _embedder_id
...
def _default_embedder(cfg: Config):
    ...
    if cfg.embed_backend == "lm_studio":
        ...
        from app.providers.lmstudio import LMStudioEmbedder
        return LMStudioEmbedder(cfg.embed_model, cfg.embed_dim, cfg.lmstudio_base_url)
    from app.providers.sentence_tf import SentenceTFEmbedder
...
def create_app(cfg: Config | None = None, *, chat_model=None, utility_model=None,
    ...
    if chat_model is None or utility_model is None:
        from app.providers.openrouter import LiteLLMChatModel, LiteLLMUtilityModel
