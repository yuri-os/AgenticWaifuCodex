...
- We don't engineer lock-in. Attachment is welcome — including a companion who openly *needs* the user and says so (the devoted-dependent pole is a feature here, not a pathology to correct → ch. 06). What we refuse is the other kind of dependence: *structural* lock-in the user can't walk away from — a trap built into the product, not a feeling inside the relationship.
