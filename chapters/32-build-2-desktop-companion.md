...
**Her thinking is now yours too.** Build #1's honest caveat was that her *mind* (the files) was yours from day one but her *thinking* was rented behind an API. Build #2 collects the debt: the local setup leads with LM Studio's OpenAI-compatible server and `CHAT_MODEL=lm_studio/…`; Ollama (`ollama/…` plus its embedding backend) is an equally supported local swap behind the exact same LiteLLM seam. The reply voice comes off your own GPU. Pull the network cable and nothing changes — that is the sovereignty bar this build clears (property 6). The escape hatch is that same seam in reverse: the model id's prefix routes the provider, so pointing `CHAT_MODEL` at a hosted `openrouter/…` model — if you don't have the GPU for a good local model, or want a stronger mind than your hardware can host — is one line plus a key. Local is the default, not a cage; the point is that sovereignty is *available*, not that a bigger model is forbidden.
...
**Discipline two — barge-in is a pipeline cancel, not a pause (§4.3).** When the user talks over her, `cancel()` must tear down TTS playback *and* the in-flight generation *together*. Here that is one `asyncio.Event`: setting it breaks the producer's `async for` over the brain's token stream — which `aclose()`s that async generator, aborting generation at the model — and stops the consumer's emit loop. Crucially, `run_turn()` creates and installs its fresh token **synchronously before it returns the async generator**. A speech frame arriving immediately after the route schedules the turn therefore cancels *this* turn, not the previous token or a token that has not yet been installed: no early-barge-in race. Cancellation has to reach all the way back, or she keeps "thinking" a reply no one is waiting for and then says it. The cancel is idempotent (the mic handler fires it on every speech frame) and scoped to the current turn.

And the failure contract, inherited verbatim from Build #1's two rules (§4.4): a **barged-in turn**, a **mid-stream model error**, or a **persistence error** all roll back the brain's uncommitted provisional user entry; closing the WebSocket's async generator does the same in its `finally` path. They persist no completed turn — no corpus line or commit from a turn that did not happen. Only a fully completed turn calls `persist()`, which is Build #1's `post_turn` pipeline (journal · index · USER.md · summary · one commit · one corpus line), scheduled off the hot path so it never delays her speaking. The corpus line is written from the *verbatim* reply, tags and all — that is what a future fine-tune should learn to produce (→ ch. 20).

`test_turn_bargein.py` pins the cancel contract: it drives the loop with a fake brain, fires `cancel()` a few tokens in, and asserts the turn ends `cancelled` (not `done`), generation stopped early, and `persist` was never called. It also covers an immediate cancel before iteration starts, plus stream and persistence errors; the integration tests additionally pin the real brain's provisional-entry rollback for cancellation and for an explicitly closed turn generator.
...
On connect she greets you first, before you type — the same definition-of-done moment as Build #1, now spoken. On a fresh Vault, `desktop/brain.py` streams `BOOTSTRAP.md`'s authored cold open verbatim; it is displayed and spoken but is not corpus-persisted, because an opener is not a turn you took. The bootstrap remains present through that cold open, then Build #1's locked `post_turn` retires it only after the first completed normal exchange. On later connects, `stream_greeting` instead assembles persona + `USER.md` + summary + a top recall against a "you just put your headset on" cue; that continuity opener is neither appended to the turn window nor persisted. If a restored Vault has history but still has `BOOTSTRAP.md`, greeting recovery force-moves the stale file into its archive safely.
...
**Desktop is optional, including under WSL.** `python -m desktop` remains the ordinary browser-hosted sanctuary; `--window` is the additive pywebview shell that makes her float. Under WSL, where that Linux native shell is not the useful desktop surface, the same flag exposes the server as needed and opens a Windows-side browser window instead. The voice loop and page are unchanged; it is a pragmatic display fallback, not a second runtime.
