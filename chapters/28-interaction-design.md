...
In the reference runtime this journey is a **consumed-once file** — `BOOTSTRAP.md` in the SOUL (→ ch. 19). It's author-shipped like the persona, but unlike the persistent files it remains active until onboarding is durably complete: the engine works its cold open and journey into the opening conversation, and retires it only after that first exchange has completed and persisted successfully (`git mv` into an `onboarded/` archive, committed — the git history is the record, no throwaway). A cancelled, disconnected, or failed first turn therefore leaves the honest cold open available; if a crash follows successful persistence but precedes the move, recovery force-safely completes the archive before another cold open can play. File presence plus the persisted completion state is the "has she been introduced yet?" state, which fits the git-backed vault where the files are the database (→ ch. 19). Restoring the file deliberately re-runs onboarding. This is also the clean split that keeps the *first-ever* meeting (the bootstrap) apart from the *return* greetings (the persistent scenario) — one is a chapter you read once, the other is the room you come home to.
