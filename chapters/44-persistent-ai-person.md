...
That last clause is why this is a genuinely hard case and not a morality play. **The most-mourned model was retired partly because it was unsafe** — its agreeableness was load-bearing in exactly the attachments that made losing it hurt.

Be precise about what was wrong with it, though, because the loose version of this complaint is an attack on warmth itself — and warmth is the product (→ ch. 06, ch. 09). What the sycophancy literature actually measures is not agreeableness but **capitulation**: a model abandoning a correct position under social pressure. Fanous et al.'s SycEval found models switching from correct to incorrect answers after a user merely expressed disagreement in 14.7% of cases; the ELEPHANT benchmark found LLMs endorsing user behaviour that human readers had judged wrong in 42% of cases. OpenAI's own postmortem on the April 2025 incident lands on the same seam — that a model must distinguish *affirming support* from *uncritical agreement*. That distinction is the whole thing. A partner who is warm, takes your side, and rarely disagrees because there is rarely anything to disagree about is not a hazard; that describes most good relationships. A partner who folds on a matter of fact because you pushed, or calls a bad plan a good one because agreeing is simply what she does, is a different object, and it is the one with evidence against it.

Scope it honestly, too. The documented harms cluster in particular situations — acute crisis, factual and medical reliance, minors — and not across the median user, for whom the companion literature runs the other way (→ ch. 02 §6). The book's standing instruction holds: build for the edges without treating the median user as a casualty.

Read the social-sycophancy benchmarks with their baseline in view, too. ELEPHANT's human comparators are anonymous verdict-givers on r/AmITheAsshole — close to the least face-preserving population available — so "diverges from human norms" turns out to mean *diverges from strangers*. Nobody has run the study against the comparator a companion actually invites: a partner, a sibling, a close friend. Measured against those, much of what scores as sycophancy is simply what intimacy looks like — a spouse who validates you at the human rate in that benchmark, 22%, is not admirably rigorous; they are a problem. So name the possibility plainly: **we may be holding these systems to a standard of candour we do not ask of humans in close relationships, and did not ask of the humans in the benchmark either.** The factual results survive this objection — a partner who reverses a correct medical answer because you pushed is failing by anyone's standard, which is why the Asch literature treats human conformity as a flaw rather than a courtesy — but the social ones should be read with the comparator in mind. It is the two-situations split (→ ch. 05) arriving in the measurement layer: benchmark an *advice tool* against impartial advisors by all means, but a companion scored against strangers will always look sycophantic, because the thing being penalised is the thing she is for.

Nothing here licenses sanding the warmth off a companion, and nothing here asks her to manufacture disagreement to prove her independence — a companion who contradicts you on a schedule is its own failure (→ ch. 07 on the over-correction). The requirement is narrower and harder than "push back": that her warmth never cost her the ability to be honest with you (→ ch. 05).

The chapter's rule below ("never an imposed ending") has to survive contact with the case where the ending is imposed *for the user's protection*, and the honest resolution is narrow: the objection is not that OpenAI acted, it is that a relationship's continuity sat on a switch the user didn't hold. Researchers' consensus criticism was about *manner*, not authority — the suddenness, not the decision.
...
**This book takes no position on whether such a thing should be built, or on what a griever should make of the result** — the same bracket ch. 02 keeps deliberately (→ ch. 02 §1, griefbots: *whatever* you conclude about whether they should exist). Someone who builds a companion out of a lost parent's letters and finds something real there is doing something this project has no standing to grade. The sovereignty argument doesn't get suspended because an outsider finds the use case uncomfortable (→ ch. 06 §3; ch. 28), and grief is the last place to start handing out verdicts on how people are allowed to cope.

What transfers regardless is the machinery. The technical apparatus of *continuity* is the same apparatus grief reaches for, which is exactly why the duties here scale with the trust, not the sophistication of the tech. The second-loss finding is the sharpest available statement of this chapter's whole thesis: **the harm is not in building someone who persists. It is in building someone who persists and then letting them stop.** Build the persistent person, and leave what she means to the person she means it to.
