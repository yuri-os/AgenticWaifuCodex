...
- How do you evaluate "personality stability" rigorously? **Less open than it was.** PersonaGym and RMTBench exist, drift is reproducibly measurable — most instruct-tuned models begin diverging from an assigned persona somewhere around 100 turns, and Lu et al. (2026) locate it mechanistically as movement along an "Assistant Axis" in activation space — and cheap statistical detectors (CUSUM) catch gradual affective flattening. What's still missing is a metric for *this* character rather than persona-adherence in general, and one that runs over months rather than turns (→ ch. 23, ch. 02 §8).
- Best representation for *relationship state* — facts? graph? narrative? mixture? (→ ch. 11, ch. 15.)
- What's the right cadence for memory consolidation — the DREAM pass (→ ch. 18, ch. 15)? The mechanism now has company (Letta's sleep-time compute, Anthropic's Dreaming) and evaluations (LongMemEval-V2, → ch. 44), so the open part has narrowed to *scheduling*: how often, triggered by what, and how you tell a consolidation that helped from one that quietly flattened her.
- **Selective forgetting.** The 2026 memory surveys converge on this as the competency current systems fail most conspicuously — they store and retrieve far better than they decide what to drop. For a companion the stakes aren't storage cost but character: what *should* she forget, what does it mean to the relationship when she does, and does a shared history need some decay to stay legible at all (→ ch. 15, ch. 11)?
- **Temporal reasoning over years of history.** "What did she believe about me last Tuesday?" Pure-vector stores are structurally poor at this, and the gap between architectures on temporal queries is large. A companion carrying a multi-year arc needs this more than an assistant does (→ ch. 15, ch. 11).
...
- **Nobody has benchmarked sycophancy against the right humans.** The social-sycophancy work measures LLMs against anonymous verdict-givers on r/AmITheAsshole — close to the least face-preserving population obtainable (→ ch. 44). The comparator a companion actually invites is a partner, a sibling, a close friend, and that study does not exist. Until it does, "more sycophantic than humans" means "more than strangers," and the distance between those two is where a companion's entire normal range lives.
- **Can a no-telemetry companion be studied longitudinally at all?** The field's own reviews name long-term data as the missing evidence base, and the ownership model (→ ch. 03 property 6) rules out the usual instrumentation by construction. Consented, user-initiated, locally-computed self-report — a diary study an owner runs on themselves and chooses to share — may be the only ethical instrument available. Designing one that yields usable data is open, and it is the same tension as the interrupt-threshold question above.

## Voice and embodiment

- Latency floor below which voice feels "too perfect / uncanny"? (Conjecture: yes.)
- **Does a body beat the novelty cliff?** The long-term HRI literature (roughly 120 long-term studies, 2003–2023) is clear that novelty decays and much less clear about what survives it. This book's answer is *a relationship* (→ ch. 43) — which is a claim, not a finding. An open, cheap body now exists to test it on (→ ch. 43, Reachy Mini as testbed). Six months of one companion in one household is the experiment, and almost nobody has run it.
- **Does a persona actually port across renderers without loss?** The entire embodiment bet is persona-as-asset, body-as-substrate (→ ch. 43, ch. 03). Nobody has shown that the *same someone* survives a move from text to voice to Live2D to a 9-DoF head — what carries, what silently doesn't, and whether the user experiences the ported version as her (→ ch. 29, ch. 25).
...
- **AR glasses: adoption is no longer the open part.** 7M+ Meta AI glasses sold in 2025, a ~950k AR-glasses forecast for 2026, and a third-party toolkit in developer preview (→ ch. 43). What's open is *design*: what a companion does on a surface that is app-invocation rather than persistent overlay, and whether a character can hold presence in glances rather than sessions.
