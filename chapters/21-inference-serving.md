...
- **llama.cpp / llamafile / Ollama / LM Studio.** Local default choices; LM Studio is the desktop-friendly OpenAI-compatible server.
...
**The local tiers are OS-portable; the remote tiers are OS-irrelevant.** GGUF + llama.cpp/Ollama/LM Studio is the portable substrate — the same quantised weights run on Linux, Windows, macOS, and ARM (Raspberry Pi, Android via termux and friends). LM Studio is an especially straightforward desktop choice where its native app is supported. MLX is the Apple-Silicon-optimized local path on macOS; TensorRT-LLM is the Nvidia/Linux performance path. On Windows, WSL2 is the practical fallback when a Linux-first server or CUDA stack is required; retain the native LM Studio or Ollama path when it better fits the machine. A remote engine, by contrast, is just an HTTPS call — the host OS doesn't matter at all. So the cross-platform story is simple: the *runtime* is portable everywhere, the *local* engines run on any desktop OS (and degrade to CPU/ARM on small devices), and where local isn't viable you swap an engine binding for a remote one without touching the rest of the system.
